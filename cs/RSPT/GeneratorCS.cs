...
namespace RSPT
{
    public class GeneratorRecursiveCS : Generator
    {
        public GeneratorRecursiveCS(Grammar grammar) : base(grammar) { }
        ...
        public override void Generate(TextWriter writer)
        {
            ...
            foreach(string include in _grammar.Includes) {
                writer.WriteLine("using {0};", include);
            }
            ...
            writer.WriteLine("");
            writer.WriteLine("public partial class {0} {{", _grammar.Class);
            writer.WriteLine("");
            ...
            foreach(SymbolNonTerm sym in _grammar.Exports) {
                writer.WriteLine("");
                writer.WriteLine("    public bool Parse_{0}({1}[] input, out {2} output, out int pos) {{", sym.Name, _grammar.Type, sym.Type);
                writer.WriteLine("        _input = input;");
                writer.WriteLine("        pos    = 0;");
                writer.WriteLine("        output = default({0});", sym.Type);
                writer.WriteLine("        return nt_{0}(ref pos, ref output) && pos == _input.Length;", sym.Name);
                writer.WriteLine("    }");
            }
            bool need_ts      = false;
            bool need_tc      = false;
            ...
            foreach(SymbolNonTerm sym in _grammar.NonTerms) {
                ...
                writer.WriteLine("");
                writer.WriteLine("    private bool nt_{0}(ref int pos, ref {1} output) {{", sym.Name, sym.Type);
                ...
                foreach(List<Symbol> rule in sym.Rules) {
                    ...
                    foreach(Symbol sym2 in rule) {
                        if(sym2 is SymbolNonTerm) {
                            ...
                            if(ins_to == null) {
                                ins_to = "output"+idx;
                                writer.WriteLine("        {0}    {1} {2} = default({1});", _indent, sym2nt.Type, ins_to);
                            }
                            writer.WriteLine("        {0}    int pos{1} = pos{2};", _indent, idx, idx-1);
                            writer.WriteLine("        {0}    if(nt_{1}(ref pos{2}, ref {3})) {{", _indent, sym2nt.Name, idx, ins_to);
                            idx++;
                            Indent(4);
                            ins_to = null;
                            ...
                            if(ins_set) { 
                                ...
                                text = string.Format("\"{0}\".ToCharArray()", Quote(sym2t.Text)); // TODO inefficient, support arrays of other types
                            } else if(ins_range) { 
                            ...
                            writer.WriteLine("        {0}    int pos{1} = pos{2};", _indent, idx, idx-1);
                            writer.WriteLine("        {0}    if({1}(ref pos{2}, {3})) {{", _indent, func, idx, text);
                            idx++;
                            Indent(4);
                            ins_set    = false;
                            ins_range  = false;
            ...
            if(need_ts) {
                writer.WriteLine("");
                writer.WriteLine("    private bool ts(ref int pos, {0}[] s) {{", _grammar.Type);
                writer.WriteLine("        foreach({0} c in s) {{", _grammar.Type);
                writer.WriteLine("            if(pos >= _input.Length || _input[pos] != c) return false;");
            ...
            if(need_tc) {
                writer.WriteLine("");
                writer.WriteLine("    private bool tc(ref int pos, {0} c) {{", _grammar.Type);
                writer.WriteLine("        if(pos >= _input.Length || _input[pos] != c) return false;");
                writer.WriteLine("        pos++;");
                writer.WriteLine("        return true;");
                writer.WriteLine("    }");
            ...
            if(need_tset) {
                writer.WriteLine("");
                writer.WriteLine("    private bool tset(ref int pos, {0}[] s) {{", _grammar.Type);
                writer.WriteLine("        for(int i = 0; i < s.Length; i++) {");
                writer.WriteLine("            if(pos < _input.Length && s[i] == _input[pos]) {");
                writer.WriteLine("                pos++;");
                writer.WriteLine("                return true;");
                writer.WriteLine("            }");
                writer.WriteLine("        }");
            ...
            if(need_trange) {
                writer.WriteLine("");
                writer.WriteLine("    private bool trange(ref int pos, {0} c1, {0} c2) {{", _grammar.Type);
                writer.WriteLine("        if(pos >= _input.Length || _input[pos] < c1 || _input[pos] > c2) return false;");
                writer.WriteLine("        pos++;");
                writer.WriteLine("        return true;");
                writer.WriteLine("    }");
            }
            if(need_tnotset) {
                writer.WriteLine("");
                writer.WriteLine("    private bool tnotset(ref int pos, {0}[] s) {{", _grammar.Type);
                writer.WriteLine("        for(int i = 0; i < s.Length; i++) {");
                writer.WriteLine("            if(pos >= _input.Length || s[i] == _input[pos]) {");
                writer.WriteLine("                return false;");
                writer.WriteLine("            }");
